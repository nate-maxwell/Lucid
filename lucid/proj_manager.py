...
def get_value_from_config(config_name: str, key: str) -> Optional[Any]:
    ...
    try:
        ...
        config_path = Path(lucid.config_paths.PROJECTS_PATH, proj, 'config', config_file)
...
class CustomInput(QtWidgets.QWidget):
    ...
    def config_path(self):
        ...
        return Path(lucid.config_paths.PROJECTS_PATH, proj, 'config')
...
class ProjSettingsTab(ConfiguratorTab):
    ...
    def save_settings(self):
        ...
        proj = lucid.environ.get_environ_var_as_list(lucid.constants.ENV_PROJECT)[0]
        output_path = Path(lucid.config_paths.PROJECTS_PATH, proj, 'config', f'{self.name}.json')
    ...
    def load_settings(self):
        ...
        if proj == '_template':
            ...
            proj_config_path = Path(lucid.config_paths.PROJECTS_PATH, proj, 'config', f'{self.name}.json')
...
class ProjectCreator(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.projs = []
        for i in lucid.io_utils.list_folder_contents(lucid.config_paths.PROJECTS_PATH):
